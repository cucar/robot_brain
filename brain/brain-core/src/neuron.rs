...
/// Per-neuron wall-clock for the 4 main ops inside process_frame.
/// Aggregated up through column/region/brain into the frame timings.
...
pub struct NeuronOpTimings {
    ...
    // Breakdown of recognize_patterns: index lookup vs candidate scoring.
    // `recognize_candidates_evaluated` counts how many child patterns were
    // scored (post-index) so we can see search-space size per frame.
