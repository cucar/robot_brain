...
- **Dyna and faithful model-based RL** (Sutton 1990; the "faithful Dyna" refinement) — the architecture's policy-training half *is* Dyna: a learned transition model (here the temporal moment graph) generates simulated experience, and planning updates the **same** action-value estimates that real experience does (see "Action reinforcement" — the as-original baseline's returns refresh the standard estimate exactly as a real execution would). The "faithful" qualifier is the load-bearing caveat: Dyna planning helps only to the extent the learned model is accurate, and acting on an unfaithful (imagined-but-wrong) model is the failure mode. Robot Brain's answer is the cold-start gate — *imagination proposes, recurrence licenses* — which withholds voting power from simulated conclusions until reality confirms them, bounding exactly the divergence faithful-Dyna work targets. Distinguished from textbook Dyna by: the model is a minted multi-parent moment graph rather than a tabular/parametric `(s,a)→s'` model, planning is salience-prioritized fan-out rather than uniform sampling, and the same substrate also does perception and episodic memory.
- **Mattar & Daw (2018)** — replay prioritized by expected value of backup (prediction error magnitude). The complement to the Dyna grounding above: Dyna says *update values from a learned model*, Mattar & Daw say *which backups to run first* — and the salience module (z-scored reward and prediction error) is this prioritization.
