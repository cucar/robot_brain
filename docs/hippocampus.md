...
what it names is present — the majority the collapse reads (D27), run by the moment
...
majority of that statement: more than half of what it names present (H2, D27).
...
> beside the action connections (§13), one exposure per frame per event dimension, at the offset the age rounds
...
every neuron, event and action alike (§13), and it holds both directions replay needs: what a situation is
...
(the cortex, §13), and replay writes imagined ones into the same connections (the hippocampus, "Action
...
- In the `process actions` call, every uncovered action activation connects to the apex events that follow it,
  at the offset its age names, beside the action connections the event activations write (H4, §13).
