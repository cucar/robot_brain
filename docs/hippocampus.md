...
> beside the action connections (§10), one exposure per frame per event dimension, at the offset the age rounds
...
every neuron, event and action alike (§10), and it holds both directions replay needs: what a situation is
...
(the cortex, §10), and replay writes imagined ones into the same connections (the hippocampus, "Action
...
- In the `process actions` call, every uncovered action activation connects to the apex events that follow it,
  at the offset its age names, beside the action connections the event activations write (H4, §10).
