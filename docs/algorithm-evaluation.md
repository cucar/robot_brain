...
and delete are what move the table between basins, and a candidate is grown greedily at every bill (R14), so
what holds a neuron in a basin is that greed — one neighbor at a time, best first, never taken back — and the
sequencing of add before retire. Choosing the subset a child should serve is facility location and is not
solved exactly at any useful size, so the gap here is a local optimum, not an oversight. **Diagnostic:** on
